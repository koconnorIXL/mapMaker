...
function getFileName(continentName) {
  continentName = continentName
    ...
    .replace('(', '')
    .replace(')', '')
  return 'topojsonDatasets/' + continentName + '_countries.json'
}
...
datasetsToModify.forEach(function(filename) {
  ...
  CONTINENTS.forEach(function(continentName) {
    ...
    topology.objects[continentName] = topology.objects.collection;
    delete topology.objects.collection;
    fs.writeFile(getFileName(continentName), JSON.stringify(topology));
  });
});
